...
async fn test_grammar_learn_basic() {
    ...
    let resp = send_unwrap(
        &handler,
        tool_call(1, "vision_grammar_learn", json!({
            "domain": "amazon.com",
            "content_map": {
...
async fn test_grammar_learn_update_existing() {
    ...
    let resp = send_unwrap(
        &handler,
        tool_call(2, "vision_grammar_learn", json!({
            "domain": "test.com",
...
async fn test_grammar_status_all() {
    ...
    let resp = send_unwrap(
        &handler,
        tool_call(2, "vision_grammar_status", json!({})),
    ).await;
...
async fn test_grammar_update_content() {
    ...
    let resp = send_unwrap(
        &handler,
        tool_call(2, "vision_grammar_update", json!({
            "domain": "test.com",
...
async fn test_grammar_pin_and_unpin() {
    ...
    send_unwrap(
        &handler,
        tool_call(1, "vision_grammar_learn", json!({ "domain": "important.com" })),
    ).await;

    // Pin
    let resp = send_unwrap(
        &handler,
        tool_call(2, "vision_grammar_pin", json!({ "domain": "important.com" })),
    ).await;
    let data = parse_tool_text(&resp);
    ...
    let resp = send_unwrap(
        &handler,
        tool_call(3, "vision_grammar_pin", json!({ "domain": "important.com", "unpin": true })),
    ).await;
    let data = parse_tool_text(&resp);
...
async fn test_dom_extract_with_grammar() {
    ...
    let resp = send_unwrap(
        &handler,
        tool_call(2, "vision_dom_extract", json!({
...
async fn test_dom_extract_without_grammar() {
    ...
    let resp = send_unwrap(
        &handler,
        tool_call(1, "vision_dom_extract", json!({
...
async fn test_intent_extract_with_grammar_route() {
    ...
    let resp = send_unwrap(
        &handler,
        tool_call(2, "vision_intent_extract", json!({
...
async fn test_intent_extract_without_route() {
    ...
    let resp = send_unwrap(
        &handler,
        tool_call(1, "vision_intent_extract", json!({
...
async fn test_intent_extract_various_intents() {
    ...
    for (intent, expected_layer) in intents {
        ...
        assert_eq!(
            ...
            "Intent '{}' should route to {}", intent, expected_layer
        );
    }
}
...
async fn test_perception_route_with_grammar() {
    ...
    let resp = send_unwrap(
        &handler,
        tool_call(2, "vision_perception_route", json!({
...
async fn test_perception_route_without_grammar() {
    ...
    let resp = send_unwrap(
        &handler,
        tool_call(1, "vision_perception_route", json!({
            "url": "https://unknown.com",
            "intent": "find_price"
        })),
    ).await;
    ...
    assert!(data["routing"]["recommendation"].as_str().unwrap().contains("vision_dom_extract"));
}
...
async fn test_stress_learn_100_grammars() {
    ...
    let resp = send_unwrap(
        &handler,
        tool_call(200, "vision_grammar_status", json!({})),
    ).await;
...
async fn test_stress_intent_extract_rapid() {
    ...
    for i in 0..200 {
        let resp = send_unwrap(
            &handler,
            tool_call(i + 10, "vision_intent_extract", json!({
...
async fn test_stress_dom_extract_alternating_grammar_no_grammar() {
    ...
    for i in (0..50).step_by(2) {
        send_unwrap(
            &handler,
            tool_call(i + 1, "vision_grammar_learn", json!({
                "domain": format!("site-{i}.com"),
                "content_map": { "field": ".sel" }
            })),
        ).await;
    ...
    for i in 0..50 {
        let resp = send_unwrap(
            &handler,
            tool_call(100 + i, "vision_dom_extract", json!({
...
async fn test_stress_grammar_update_overwrite_cycle() {
    ...
    for i in 0..50 {
        send_unwrap(
            &handler,
            tool_call(i + 10, "vision_grammar_update", json!({
                "domain": "evolving.com",
    ...
    let resp = send_unwrap(
        &handler,
        tool_call(100, "vision_grammar_get", json!({ "domain": "evolving.com" })),
    ).await;

    let data = parse_tool_text(&resp);
    let selector = data["grammar"]["content_map"]["price"]["selector"].as_str().unwrap();
