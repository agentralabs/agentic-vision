...
impl ReplState {
    pub fn new() -> Self {
        Self { file_path: None, json: false }
    }
    ...
    pub fn dispatch(&mut self, line: &str) -> VisionResult<()> {
        ...
        match cmd {
            ...
            "json" => { self.json = !self.json; println!("JSON output: {}", if self.json { "on" } else { "off" }); Ok(()) }
            "clear" => { print!("\x1B[2J\x1B[1;1H"); Ok(()) }
            "help" => { self.print_help(); Ok(()) }
            "exit" | "quit" => { println!("Goodbye."); std::process::exit(0); }
            _ => {
    ...
    fn require_file(&self) -> VisionResult<&PathBuf> {
        self.file_path.as_ref().ok_or_else(|| {
            crate::types::VisionError::Io(std::io::Error::new(
                std::io::ErrorKind::NotFound, "No file loaded. Use /create <path> or /load <path> first."))
        })
    }

    fn cmd_create(&mut self, args: &[&str]) -> VisionResult<()> {
        let path = args.first().ok_or_else(|| crate::types::VisionError::Io(
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "Usage: /create <path> [dimension]")))?;
    ...
    fn cmd_load(&mut self, args: &[&str]) -> VisionResult<()> {
        let path = args.first().ok_or_else(|| crate::types::VisionError::Io(
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "Usage: /load <path>")))?;
        let p = PathBuf::from(path);
    ...
    fn cmd_info(&self) -> VisionResult<()> { commands::cmd_info(self.require_file()?, self.json) }

    fn cmd_capture(&self, args: &[&str]) -> VisionResult<()> {
        let p = self.require_file()?;
        let source = args.first().ok_or_else(|| crate::types::VisionError::Io(
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "Usage: /capture <image_path>")))?;
    ...
    fn cmd_similar(&self, args: &[&str]) -> VisionResult<()> {
        let id: u64 = args.first().and_then(|s| s.parse().ok()).ok_or_else(|| crate::types::VisionError::Io(
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "Usage: /similar <capture_id> [top_k]")))?;
    ...
    fn cmd_compare(&self, args: &[&str]) -> VisionResult<()> {
        if args.len() < 2 { return Err(crate::types::VisionError::Io(
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "Usage: /compare <id_a> <id_b>"))); }
        let id_a: u64 = args[0].parse().map_err(|_| crate::types::VisionError::Io(
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid capture ID")))?;
        let id_b: u64 = args[1].parse().map_err(|_| crate::types::VisionError::Io(
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid capture ID")))?;
    ...
    fn cmd_diff(&self, args: &[&str]) -> VisionResult<()> {
        if args.len() < 2 { return Err(crate::types::VisionError::Io(
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "Usage: /diff <id_a> <id_b>"))); }
        let id_a: u64 = args[0].parse().map_err(|_| crate::types::VisionError::Io(
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid capture ID")))?;
        let id_b: u64 = args[1].parse().map_err(|_| crate::types::VisionError::Io(
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid capture ID")))?;
    ...
    fn cmd_health(&self) -> VisionResult<()> { commands::cmd_health(self.require_file()?, 168, 0.45, 20, self.json) }

    fn cmd_link(&self, args: &[&str]) -> VisionResult<()> {
        if args.len() < 2 { return Err(crate::types::VisionError::Io(
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "Usage: /link <capture_id> <memory_node_id>"))); }
        let cid: u64 = args[0].parse().map_err(|_| crate::types::VisionError::Io(
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid capture ID")))?;
        let mid: u64 = args[1].parse().map_err(|_| crate::types::VisionError::Io(
            std::io::Error::new(std::io::ErrorKind::InvalidInput, "Invalid memory node ID")))?;
        commands::cmd_link(self.require_file()?, cid, mid, self.json)
    }

    fn cmd_stats(&self) -> VisionResult<()> { commands::cmd_stats(self.require_file()?, self.json) }
...
fn suggest_command(input: &str) -> Option<&'static str> {
    const CMDS: &[&str] = &["create","load","info","capture","query","similar","compare","diff","health","link","stats","export","json","clear","help","exit"];
    let mut best: Option<(&str, usize)> = None;
    for &cmd in CMDS {
        let dist = levenshtein(input, cmd);
        if dist <= 2 && (best.is_none() || dist < best.unwrap().1) { best = Some((cmd, dist)); }
    }
    best.map(|(s, _)| s)
}
